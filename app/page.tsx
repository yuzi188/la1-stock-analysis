...
import { usePaperTrading } from "./trading-core/hooks/usePaperTrading";
  ...
  return (
    <main className="la1-shell">
      <GlobalStatusBar
        ...
        dailyPnlPct={paper.dailyPnlPct}
        robotState={paper.robotState}
        openPositions={paper.account.positions.length}
        onEmergencyStop={() => paper.updateConfig({ emergencyStop: true })}
        onOpenTrade={() => setActivePage("trade")}
      />
      ...
      <section className="workspace">
        ...
        {activePage === "ai" ? (
          <section className="dashboard ai-grid">
            ...
            <TradePlanCard
                  ...
                  ? toStructuredDecision(
                      {
                        ...
                        analysis: {
                          ...
                          stance: analysis.stance as "偏多" | "中性" | "偏保守",
                        ...
                        generatedAt: analysisMeta?.generatedAt,
                      },
                      quote.symbol,
                    )
                  : null
              ...
              onPaperBuy={() => {
                if (!analysis || !quote?.price) return;
                paper.paperBuyFromAnalyze(
                  {
                    ok: true,
                    quote,
                    analysis: {
              ...
              onAddToRobot={() => {
                ...
                const list = paper.config.whitelist.includes(quote.symbol)
                  ? paper.config.whitelist
                  : [...paper.config.whitelist, quote.symbol];
                paper.updateConfig({ whitelist: list });
              }}
            />
          </section>
        ) : null}
        ...
        {activePage === "trade" ? (
          <TradePage
            ...
            equity={paper.account.equity}
            dailyPnlPct={paper.dailyPnlPct}
            positions={paper.account.positions.map((p) => ({
              symbol: p.symbol,
              quantity: p.quantity,
              avgPrice: p.avgPrice,
              lastPrice: p.avgPrice,
              stopLoss: p.stopLoss,
              takeProfit: p.takeProfit,
            }))}
            orders={paper.account.orders.map((o) => ({
              id: o.id,
              symbol: o.symbol,
              side: o.side,
              quantity: o.quantity,
            ...
            config={paper.config}
            robotRunning={paper.robotRunning}
            robotCycles={paper.robotCycles}
            lastCycleAt={paper.lastCycleAt}
            onStartRobot={() => {
              paper.updateConfig({ emergencyStop: false, enabled: true });
              paper.setRobotRunning(true);
            }}
            onStopRobot={() => paper.setRobotRunning(false)}
            onEmergencyStop={() => {
              paper.updateConfig({ emergencyStop: true });
              paper.setRobotRunning(false);
            }}
            onUpdateConfig={(partial) => paper.updateConfig(partial)}
            onClosePosition={(symbol) => {
              const price = quote?.symbol === symbol && quote.price ? quote.price : paper.account.positions.find((p) => p.symbol === symbol)?.avgPrice ?? 0;
              paper.closePosition(symbol, price);
            }}
          />
        ) : null}
